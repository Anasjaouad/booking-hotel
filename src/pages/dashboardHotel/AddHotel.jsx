...
function AddHotel(){
    ...
    const [inputData, setInputData] =useState({
        ...
        amenities:{
            "Free wi-fi":false,
            "Breakfeast Included":false,
            "Airoport Shuttle":false,
            "Swimming Pool":false
        }
    })
    return<>
    <form className="mt-4 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 my-4">
            {Object.keys(image).map((key)=>(
                <label htmlFor={`hotelImage${key}`} key={key} className="flex flex-col items-center cursor-pointer">
                    <img src={image[key] ?URL.createObjectURL(image[key]) :uploadArea} alt={`Hotel Image ${key}`} className="max-h-13 cursor-pointer opacity-80" />
        ...
        <div className="w-full flex sm:flex-col sm-gap-4 mt-4">
            ...
            <div className="">
                ...
                <input type="number" placeholder="0" className="border border-gray-300 mt-2 rounded w-25" 
                value={inputData.pricePerNight} onChange={(e)=>setInputData({...inputData,pricePerNight:e.target.value})}/>
            </div>
        </div>
        ...
        <button className="bg-amber-500 text-white rounded p-2 mt-4 hover:bg-amber-600 transition-colors">Add Hotels</button>
    </form>
    </>
}
